...
func TestSlackTokenAuthenticate(t *testing.T) {
	...
	slack_api_token := "xoxp-5228148520-5228148525-1323104836872-10674849628c43b9d4b4660f7f9a7b65" //nolint:gosec // test token
	...
	for _, sm := range Matchers() {
		if sm.Type() == "slack_api_token" {
			...
			fmt.Printf("TestSlackTokenAuth authentication result: [%s]", *result) //nolint:forbidigo // acceptable
		}
...
func TestStripeAuthOk(t *testing.T) {
	fmt.Println()                                      //nolint:forbidigo // acceptable
	stripeApiKey := "sk_test_tR3PYbcVNZZ796tH88S4VQ2u" //nolint:gosec // test token
	...
	for _, sm := range Matchers() {
		if sm.Type() == "stripe_api_key" {
			...
			fmt.Printf("TestStripeAuthOk authentication result: [%s]", *result) //nolint:forbidigo // acceptable
		}
...
func TestStripeAuthFail(t *testing.T) {
	fmt.Println()                                      //nolint:forbidigo // acceptable
	stripeApiKey := "sk_live_tR3PYbcVNZZ796tH88S4VQ2u" //nolint:gosec // test token
	...
	for _, sm := range Matchers() {
		if sm.Type() == "stripe_api_key" {
			...
			fmt.Printf("TestStripeAuthFail authentication result: [%s]", *result) //nolint:forbidigo // acceptable
		}
	}
}
...
func TestMailChimpAuthFalse(t *testing.T) {
	...
	for _, sm := range Matchers() {
		if sm.Type() == "mailchimp_access_key" {
			...
			fmt.Printf("TestMailChimp authentication result: [%s]", *result) //nolint:forbidigo // acceptable
		}
	}
}
...
func TestAWSAuthFalse(t *testing.T) {
	...
	src := "AKIAQGDRKHTKEHWDVRFB   HZjvc32t8fOFrYYD2RyGFUlPXeZHHZne+u0K/Waa" //nolint:gosec // test token

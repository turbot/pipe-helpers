...
import (
	...
	putils "github.com/turbot/pipe-helpers/utils"
	pversionfile "github.com/turbot/pipe-helpers/versionfile"
...
func updatePluginVersionFiles(ctx context.Context, image *OciImage[*PluginImage, *PluginImageConfig], constraint string, metadata map[string][]string) error {
	...
	v, err := pversionfile.LoadPluginVersionFile(ctx)
	if err != nil {
		return err
	}
	...
	if !ok {
		installedVersion = pversionfile.EmptyInstalledVersion()
	}
